...
An off-canvas sidebar Next.js component with a collection of effects and styles using CSS transitions and SVG path animations.

## Demo & examples

Live demo: [haqqman.github.io/next-burger-menu](haqqman.github.io/next-burger-menu/)
...
The easiest way to use **Next Burger Menu** is to install it using pnpm and include it in your own Next/React build process (using [Browserify](https://browserify.org), [Webpack](https://webpack.github.io/), etc).

You can also use the standalone build by including `dist/next-burger-menu.js` in your page. If you use this, make sure you have already included React, and it is available as a global variable.

Version 3.x uses Hooks, so if you're using React 16.8+:
```
pnpm install next-burger-menu --save
```

If you're using an earlier version of React:
```
pnpm install next-burger-menu@^2.9.2 --save
```
...
class Example extends React.Component {
  ...
  render () {
    // NOTE: You also need to provide styles, see https://github.com/haqqman/next-burger-menu#styling
    return (
      <Menu>
...
The example above imported `slide` which renders a menu that slides in on the page when the burger icon is clicked. To use a different animation you can substitute `slide` with any of the following (check out the [demo](https://example.com) to see the animations in action):
...
* **Page wrapper** - an element wrapping the rest of the content on your page (except elements with fixed positioning - see [the wiki](https://example.com/next-burger-menu/wiki/FAQ#i-have-a-fixed-header-but-its-scrolling-with-the-rest-of-the-page-when-i-open-the-menu) for details), placed after the menu component
...
Check the FAQ (https://example.com/next-burger-menu/wiki/FAQ) to see if your question has been answered already, or [open a new issue](https://github.com/haqqman/next-burger-menu/issues).
